...
class COLIEE_train(Dataset):
    def __init__(self, train_path, tokenizer, max_length=550):
        ...
        for sample in dataset:
            # print(sample)
            self.post_list.append('Query:' + sample["query"] + 'Document:' + sample["doc"] + 'Relevant:')
            self.label_list.append(sample["label"])
        self.tokenizer = tokenizer
        self.max_length = max_length
        self.input_ids = []
        self.attn_masks = []
    ...
    def __getitem__(self, idx):
        ...
        input_ids = torch.tensor(encodings_dict["input_ids"])
        attn_masks = torch.tensor(encodings_dict["attention_mask"])
        label_encodeing = self.tokenizer(self.label_list[idx], truncation=True, max_length=self.max_length, padding="max_length")
        label_input_ids = torch.tensor(label_encodeing["input_ids"])
        return {
            "input_ids": input_ids,
            "attention_mask": attn_masks,
