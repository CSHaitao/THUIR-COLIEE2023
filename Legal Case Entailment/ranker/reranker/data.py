...
class GroupedTrainDataset(Dataset):
    ...
    def create_one_example(self, qry_encoding, doc_encoding):

        qry_encoding = self.tok.encode(qry_encoding,max_length=250,truncation=True)
        # print(qry_encoding)
        doc_encoding = self.tok.encode(doc_encoding,max_length=250,truncation=True)
        # qry_encoding = qry_encoding.split(' ')[:255]
        # qry_encoding = " ".join(qry_encoding)
        # doc_encoding = doc_encoding.split(' ')[:255]
        # doc_encoding = " ".join(doc_encoding)
        # print(qry_encoding)
