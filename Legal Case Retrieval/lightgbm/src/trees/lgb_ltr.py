...
def calc_dcg(query_list, K=5, prefix=''):
...
def custom_f1_eval(preds, train_data):
    ...
    recall = num_TP / len(y_train[y_train == 1])
...
def train(x_train, y_train, q_train, x_test, y_test, q_test, model_save_path):
    ...
    params = {
        ...
        'max_position': 5,  # @NDCG 位置优化
        'metric_freq': 1,  # 每隔多少次输出一次度量结果
        'train_metric': True,  # 训练时就输出度量结果
        'ndcg_at': [5],
        'max_bin': 255,  # 一个整数，表示最大的桶的数量。默认值为 255。lightgbm 会根据它来自动压缩内存。如max_bin=255 时，则lightgbm 将使用uint8 来表示特征的每一个值。
        'num_iterations': 1000,  # 迭代次数，即生成的树的棵数
        'learning_rate': 0.01,  # 学习率 [0.01, 0.015, 0.025, 0.05, 0.1],一般先选一个较小的学习速率，再按情况逐步增大
        'num_leaves': 30,  # 叶子数 此参数的数值应该小于 2max_depth2^{max\_depth}2max_depth
        # 'max_depth':6,
    ...
    gbm = lgb.train(params, train_data, valid_sets=valid_data,early_stopping_rounds=100,evals_result=evals_result) # 这里valid_sets可同时加入train_data，val_data
...
if __name__ == '__main__':
    ...
    if sys.argv[1] == '-process':
        ...
        print(len(y_pred))
        qid_idx_writer = open('/output.tsv','w')

        order = 'valid_order.txt'
        file = open(order,'r')
        i = 0
        ...
        for line in tqdm(file.readlines()):
            ...
            qid = line[1]
            pid = line[2]
            qid_idx_writer.write(f'{qid}\t{pid}\t{y_pred[i]}\n')
            i = i + 1
        ...
        # ndcg
        test_path = base_path + '/data/valid/valid.txt'#评估测试数据的平均ndcg
